...
try {
    ...
    foreach ($recurring as $exp) {
        ...
        if ($next_date && $next_date <= $today) {
            ...
            if (!$checkStmt->fetch()) {
                // Clone it
                $insertStmt = $pdo->prepare("INSERT INTO expenses (category_id, vendor_id, po_id, name, amount, expense_date, notes, status, created_by, is_recurring, recurrence_interval) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $insertStmt->execute([
                    $exp['category_id'],
                    $exp['vendor_id'],
                    $exp['po_id'],

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Uniqueness check for slug (exclude current role)
    $chk = $conn->prepare("SELECT id FROM roles WHERE slug = ? AND id <> ? LIMIT 1");
    ...
    try {
        ...
        try {
            ...
            if (!empty($selected)) {
                ...
                foreach ($selected as $pid) { $ins->bind_param('ii', $id, $pid); $ins->execute(); }
                $ins->close();
            }
            $conn->commit();
            ...
            $conn->rollback();
            setAlert('danger', 'Role updated, but failed to save permissions: ' . $e->getMessage());
        }
        redirect('index.php');
    } catch (Throwable $e) {
        setAlert('danger', 'Failed to update role: ' . $e->getMessage());
        redirect('edit.php?id=' . $id);
    }
}
...
$assigned = $conn->query('SELECT permission_id FROM role_permissions WHERE role_id=' . $id)->fetch_all(MYSQLI_ASSOC);

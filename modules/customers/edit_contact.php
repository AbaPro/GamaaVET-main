...
// Get contact info
$contact_sql = "SELECT * FROM customer_contacts WHERE id = ? AND customer_id = ?";
$contact_stmt = $conn->prepare($contact_sql);
$contact_stmt->bind_param("ii", $contact_id, $customer_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Update contact
        $update_sql = "UPDATE customer_contacts SET 
        ...
        if (!$is_primary && $contact['is_primary']) {
            $check_primary_sql = "SELECT COUNT(*) as count FROM customer_contacts WHERE customer_id = ? AND is_primary = 1";
            $check_primary_stmt = $conn->prepare($check_primary_sql);
            $check_primary_stmt->bind_param("i", $customer_id);
            ...
            if ($primary_count === 0) {
                $set_primary_sql = "UPDATE customer_contacts SET is_primary = 1 WHERE id = ?";
        ...
        logActivity("Updated contact ID: $contact_id for customer ID: $customer_id");
    } catch (Exception $e) {
        $conn->rollback();
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2>Edit Contact</h2>
    <a href="contacts.php?id=<?php echo $customer_id; ?>" class="btn btn-secondary">Back to Contacts</a>
</div>

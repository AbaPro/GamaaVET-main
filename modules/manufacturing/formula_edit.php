        ...
        try {
            if ($id > 0) {
                $stmt = $pdo->prepare("
                    ...
                    SET customer_id = ?, name = ?, description = ?, batch_size = ?, components_json = ?, instructions = ?, is_active = ?
                    WHERE id = ?
                ");
                $stmt->execute([$customerId, $name, $description, $batchSize, $componentsJson, $instructions, $isActive, $id]);
                setAlert('success', 'Formula updated successfully.');
            } else {
                $stmt = $pdo->prepare("
                    INSERT INTO manufacturing_formulas 
                        (customer_id, name, description, batch_size, components_json, instructions, is_active) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([$customerId, $name, $description, $batchSize, $componentsJson, $instructions, $isActive]);
